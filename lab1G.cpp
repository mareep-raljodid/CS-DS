...
int main()
{
  ...
  // as.id[] = {914, 915, 916, 917, 918, 919, 920, 921, 922};

   // Array with number of units sold for each product
   //as.units[NUM_PRODS] = {842, 416, 127, 514, 437, 269, 97,
 //                          492, 212};

   // Array with product prices
  // as.prices[NUM_PRODS] = {12.95, 14.95, 18.95, 16.95, 21.95,
   ...
   calcSales(as.units, as.prices, as.sales, NUM_PRODS);
   ...
   dualSort(as.id, as.sales, NUM_PRODS);

   // Set the numeric output formatting.
   cout << setprecision(2) << fixed << showpoint;

   // Display the products and sales amounts.
   showOrder(as.sales, as.id, NUM_PRODS);

   // Display total units sold and total sales.
   showTotals(as.sales, as.units, NUM_PRODS);
   return 0;
}
